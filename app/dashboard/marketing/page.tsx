    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b">
        <div className="px-6 py-4 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Niche Marketing</h1>
            <p className="text-gray-600">Build personalized marketing plans for target groups</p>
          </div>
        </div>
      </header>
              ...
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className="text-center p-4 bg-blue-50 rounded-lg">
                    ...
                    <div className="text-sm font-medium text-blue-700">Target Audience</div>
                    <div className="text-xs text-blue-600 mt-1">{plan.audience}</div>
                  </div>
                  <div className="text-center p-4 bg-green-50 rounded-lg">
                    <DollarSign className="h-6 w-6 text-green-600 mx-auto mb-2" />
                    <div className="text-sm font-medium text-green-700">Budget</div>
                    <div className="text-lg font-bold text-green-600">{plan.budget}</div>
                  </div>
                  <div className="text-center p-4 bg-purple-50 rounded-lg">
                    <Calendar className="h-6 w-6 text-purple-600 mx-auto mb-2" />
                    <div className="text-sm font-medium text-purple-700">Timeline</div>
                    <div className="text-sm text-purple-600">{plan.timeline}</div>
                  </div>
                  <div className="text-center p-4 bg-orange-50 rounded-lg">
                    <Target className="h-6 w-6 text-orange-600 mx-auto mb-2" />
                    <div className="text-sm font-medium text-orange-700">Channels</div>
                    <div className="text-xs text-orange-600">{plan.channels.join(", ")}</div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {plan.kpis.map((kpi, index) => (
                      <div key={index} className="p-3 bg-gray-50 rounded-lg border">
                        <div className="text-sm text-gray-700">{kpi}</div>
                      </div>
                    ))}
                  </div>
                </div>
